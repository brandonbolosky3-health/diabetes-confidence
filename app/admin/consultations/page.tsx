      ...
      <div>
        ...
        {suffering.length > 0 && (
          <div className="mb-2">
            ...
            <div className="flex flex-wrap gap-1.5">
              {suffering.map((s) => (
                <span key={s} className="bg-amber-100 text-amber-800 text-[0.7rem] font-medium px-2 py-0.5 rounded-full">{s}</span>
              ))}
            </div>
          </div>
        )}
        {impacts.length > 0 && (
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {impacts.map((s) => (
                <span key={s} className="bg-red-100 text-red-700 text-[0.7rem] font-medium px-2 py-0.5 rounded-full">{s}</span>
              ))}
            </div>
          </div>
        )}
...
export default function ConsultationsAdminPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (data) setRows(data as FormRow[]);
      setLoading(false);
    })();
  }, []);
    ...
    <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 py-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[0.85rem] text-[color:var(--muted-foreground)] mt-0.5">{rows.length} total submissions</p>
        </div>
      </div>
